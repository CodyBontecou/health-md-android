...
| Withings `withings/activity_summary` | `/v2/measure?action=getactivity` | none, explicit | captured-zone day parameters | true |
| Withings `withings/sleep_summary` | `/v2/sleep?action=getsummary` | none, explicit | captured-zone day parameters | true |
| Withings `withings/measures` | `/measure?action=getmeas` | none, explicit | epoch-second `[start,end)` parameters | false |

## Fidelity and unsupported providers

Fitbit, Oura, WHOOP, and Withings declare `native_api_payload`. Their successful response bytes, JSON whitespace/order/unknown fields, native units, content type/charset, allowlisted response headers, and sanitized request metadata are preserved. Endpoint identifiers are provider-scoped hashes, never paths or URLs. Cursor/token values, `cycleId`, data-field lists, OAuth material, cookies, arbitrary errors, and arbitrary headers are excluded.
