...
The page also carries provider ID, stable logical endpoint key, opaque endpoint digest, positive page ordinal, fetch instant, 2xx status, sanitized content type/charset, allowlisted response headers, sanitized query metadata, and `serverAggregation`. The exact response bytes can contain response-internal pagination cursor/token fields and must be treated as sensitive; exact fidelity forbids deleting or rewriting them. Outside `responseBytesBase64` and its exact `responseText`, a page MUST NOT contain OAuth/access/refresh tokens, Authorization, cookies, secret query/cursor values, arbitrary response headers/errors, stack traces, or a full URL. Tokens/cursors can drive a later request but are omitted from query metadata, logs, errors, endpoint identifiers, and headers. Identity is `cloud:<provider>:<endpointKey>:<pageOrdinal>:<responseSha256>`; no random ID or normalized `HealthData` serialization is allowed.
...
Cloud page records preserve endpoint response bytes, including provider JSON whitespace, ordering, unknown fields, native units, and any response-internal cursor fields. They do not claim each response is record-level: endpoints marked `serverAggregation=true` are provider summaries and remain labeled as such. Fitbit explicitly has no endpoint pagination. Oura and WHOOP use capped, cycle-detected next-token paging; Withings `getactivity`, `getsummary`, and `getmeas` use capped, cycle-detected `body.more`/`body.offset` paging. Withings top-level `status` must be zero before capture, so HTTP-200 application error envelopes are not records. WHOOP recovery pages are deterministic fan-out requests whose IDs are discovered only from captured cycle pages.
